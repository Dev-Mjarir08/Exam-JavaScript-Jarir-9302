    ...
    cartData.forEach((product) => {
        ...
        col.innerHTML = `
            <div class = "border border-1 p-4 mb-5 rounded shadow">
                ...
                <p class="fw-semibold text-light me-3">${ProductName}</p>
                <p class="fw-semibold text-light me-3">₹${ProductPrice}</p>
              </div>
              <div class="btns d-flex justify-content-center align-items-center mb-5">              
              <button class="btn btn-outline-light me-2 fs-5 py-1 px-3" onclick ="decrease(${id})">-</button>
              <span class="text-light">${qty}</span>
              <button class="btn btn-outline-light ms-2 fs-5 py-1 px-3" onclick ="increase(${id})">+</button>
              </div>
              <button class="btn btn-outline-light" onclick = "removeItem(${id})";>Remove Product</button>
            </div>
